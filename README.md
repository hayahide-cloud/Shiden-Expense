...
- **手動トリミング（iOSの「書類をスキャン」風）**: 撮影・選択した画像の上に四隅の
  ドラッグハンドルを表示し、領収書の角に合わせて切り抜ける。初期位置は背景検出で
  自動配置されるが、最終確認は人間が行うため誤検出で金額等が欠落する事故が起きない。
  ...
  「影消去」と同じ原理）と、明るさ・コントラストの自動補正（オートレベル）で
  影のかかった紙も均一な白にしてからOCR・保存に使う。切り抜きは高解像度の元画像（長辺3600px）に対して
